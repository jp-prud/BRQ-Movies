...
function listFavoriteMovies(): Promise<FavoriteMovieProps[] | null> {
  ...
  async function getMovieById(movieId: MovieProps['id']): Promise<MovieProps> {
    ...
    const { data } = await HttpClient.get<MoviePropsAPI>(
      `/movie/${movieId}?language=en-US`,
    );
    return toPresentation(data);
  }

  async function favoritedMovieById(
    movieId: MovieProps['id'],
  ): Promise<FavoriteMovieProps | null> {
    const favoriteMovies =
      (await storage.getItem<FavoriteMovieProps[]>(
        StorageKeys.FavoriteMovies,
      )) || [];
    return favoriteMovies.find(movie => movie.id === movieId) || null;
  }

  async function favoriteMovie(
    favoriteMovie: Pick<MovieProps, 'id' | 'posterPath'>,
  ): Promise<void> {
    const favoriteMovies =
      (await storage.getItem<FavoriteMovieProps[]>(
        StorageKeys.FavoriteMovies,
